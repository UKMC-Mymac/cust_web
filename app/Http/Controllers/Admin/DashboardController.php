	...
	public function index()
   	{
		...
		$data['approved_applications'] = Application::where('status', '2')->get();
		$data['total_application_fees'] = Application::where('pay_status', '1')->sum('fee_amount');

		$monthly_application_fees = [];
		for($a = 1; $a <= $month; $a++){
			$monthly_application_fees[] = Application::where('pay_status', '1')
				->whereYear('updated_at', $year)
				->whereMonth('updated_at', $a)
				->sum('fee_amount');
		}
