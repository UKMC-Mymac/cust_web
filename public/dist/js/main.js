  ...
  /*---------- 01. On Load Function ----------*/
		...
		return this.each(function () {
			...
			$(itemHasChildren).each(function () {
				$(this).on("click", function (e) {
					...
					if ($(e.target).hasClass(opt.meanExpandClass) || $(e.target).closest("." + opt.meanExpandClass).length || href === "#" || href === "" || href === "javascript:void(0)") {
						e.preventDefault();
	...
	function createScrollTrigger() {
		...
		if (
			...
			document.querySelector(".video-trigger-thumb")
		) {
	...
	let thPositionSticky = function ($scope, $) {
		...
		function thCheckSticky() {
			$images.each(function () {
				...
				if (
					$scrollTop + $windowHeight > currentOffsetTop + 340 &&
					$scrollTop < currentOffsetTop + $current.outerHeight()
				) {
	...
	function initializeLenis() {
		...
		// ✅ Moved ticker INSIDE initializeLenis()
	...
	//   }
		...
		function updateMenuOverflow($item) {
			...
			$submenu.removeClass('submenu-open-left submenu-open-left-nested');

			var parentRect = $item.get(0).getBoundingClientRect();
			var submenuWidth = $submenu.outerWidth() || 250;

			var depth = $item.parentsUntil('.main-menu', 'ul').length || 1;

			if (parentRect.right + submenuWidth > window.innerWidth - 12) {
				if (depth > 1) {
					$submenu.addClass('submenu-open-left-nested');
				} else {
		...
		function refreshMenuOverflow() {
			$('.main-menu > ul > li.menu-item-has-children, .main-menu > ul > li.has-sub').each(function () {
				var $submenu = $(this).children('ul').first();

				if ($submenu.length && $submenu.is(':visible')) {
					updateMenuOverflow($(this));
				}
			});
		}

		// Bind to all levels so nested flips are handled
		$(document).on('mouseenter focusin', '.main-menu li.menu-item-has-children, .main-menu li.has-sub', function () {
			updateMenuOverflow($(this));
		});
