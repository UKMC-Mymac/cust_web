...
Route::middleware(['XSS'])->namespace('Web')->group(function () {
    ...
    // Custom Landing Page - Home Route
    // Route::get('/', function () {
    ...
    Route::get('/academics/departments/{slug}', 'CourseController@departmentPrograms')->name('department.programs');
    Route::get('/academics/program/{slug}', 'CourseController@programSingle')->name('program.single');
    Route::get('/academics/programs', 'CourseController@allPrograms')->name('program');
    // Campus Life Route
    Route::get('/campus-life', 'CampuslifeController@index')->name('campus-life');
    Route::get('/campus-life/{slug}', 'CampuslifeController@show')->name('campus-life.single');
    // Testimonial Route
    Route::get('/testimonials', 'TestimonialsController@index')->name('testimonial');
    Route::get('/testimonials/{slug}', 'TestimonialsController@show')->name('testimonial.single');
    // Event Route
    ...
    Route::get('/result/{result}', 'ResultController@show')->name('result.show');
    // ClassSchedule Route
    Route::get('/class-schedule', 'ClassScheduleController@index')->name('class-schedule');
    Route::get('/class-schedule/{classSchedule}', 'ClassScheduleController@show')->name('class-schedule.show');
    // AcademicCalendar Route
    Route::get('/academic-calendar', 'AcademicCalendarController@index')->name('academic-calendar');
    Route::get('/academic-calendar/{academicCalendar}', 'AcademicCalendarController@show')->name('academic-calendar.show');
    // Page Route
    Route::get('/page/{slug}', 'PageController@show')->name('page.single');
    // Members Route
    Route::get('/members', 'MembersController@index')->name('members');
    Route::get('/members/{slug}', 'MembersController@show')->name('members.single');
    ...
    Route::get('application/success/{id}', 'ApplicationController@success')->name('application.success');

    // Public Live Chat Routes
    Route::post('chat/session', 'PublicChatController@startSession')->name('chat.session');
    Route::get('chat/{chatId}/messages', 'PublicChatController@messages')->name('chat.messages');
    Route::post('chat/{chatId}/message', 'PublicChatController@sendMessage')->name('chat.message.send');
    Route::post('chat/{chatId}/leave', 'PublicChatController@leaveChat')->name('chat.leave');
...
Route::middleware(['XSS'])->group(function () {
    ...
    Route::post('filter-batch-by-program', 'FilterController@filterBatchByProgram')->name('filter-batch-by-program');
...
Route::middleware(['XSS'])->name('payment.')->namespace('Payment')->prefix('payment')->group(function () {
    ...
    // bKash Routes
    Route::post('bkash/process', 'BkashController@process')->name('bkash.process');
    Route::get('bkash/success', 'BkashController@success')->name('bkash.success');
    Route::get('bkash/cancel', 'BkashController@cancel')->name('bkash.cancel');

    // Nagad Routes
    Route::post('nagad/process', 'NagadController@process')->name('nagad.process');
    Route::get('nagad/success', 'NagadController@success')->name('nagad.success');
    Route::get('nagad/cancel', 'NagadController@cancel')->name('nagad.cancel');

    // SSLCommerz Routes
    Route::post('sslcommerz/process', 'SslCommerzController@process')->name('sslcommerz.process');
...
Route::middleware(['auth:web', 'XSS'])->name('admin.')->namespace('Admin')->prefix('admin')->group(function () {
    ...
    // Page editor image upload (used by WYSIWYG)
    Route::post('web/page/upload', 'Web\PageController@upload')->name('page.upload');
    Route::get('web/members/search', 'Web\MembersController@search')->name('web.members.search');
    Route::resource('web/members', 'Web\MembersController');

    // Live Chat Routes
    Route::get('chat', 'LiveChatController@index')->name('live-chat.index');
    Route::get('chat/threads', 'LiveChatController@threads')->name('live-chat.threads');
    Route::delete('chat/all', 'LiveChatController@destroyAll')->name('live-chat.destroy-all');
    Route::get('chat/{chatId}/messages', 'LiveChatController@messages')->name('live-chat.messages');
    Route::post('chat/{chatId}/message', 'LiveChatController@sendMessage')->name('live-chat.message.send');
    Route::patch('chat/{chatId}/status', 'LiveChatController@updateStatus')->name('live-chat.status');
    Route::delete('chat/{chatId}', 'LiveChatController@destroy')->name('live-chat.destroy');

    // Student Routes
    Route::get('admission/application-transaction', 'ApplicationController@transaction')->name('application.transaction');
    Route::post('admission/application/store-offline', 'ApplicationController@storeOffline')->name('application.store-offline');
    ...
    Route::resource('communicate/result', 'ResultController');
    Route::resource('communicate/class-schedule', 'ClassScheduleController');
    Route::resource('communicate/academic-calendar', 'AcademicCalendarController');
    ...
    // Web Section Settings
    Route::get('setting/web-sections', 'WebSectionController@index')->name('web-sections.index');
    Route::post('setting/web-sections/store', 'WebSectionController@store')->name('web-sections.store');

    // Custom URL Settings
    Route::resource('setting/custom-url', 'CustomUrlController');
    ...
    Route::prefix('web')->namespace('Web')->group(function () {
        ...
        Route::resource('campus-life', 'CampusLifeController');
        Route::resource('why-choose-us', 'WhyChooseUsController');
        Route::resource('apply', 'ApplyController');
        Route::resource('about-us', 'AboutUsController');
        Route::resource('course', 'CourseController');
        Route::resource('school', 'SchoolController');
        Route::resource('department', 'DepartmentController')->names('web-department');
        ...
        Route::post('navbar/reorder', 'NavbarController@reorder')->name('navbar.reorder');
        ...
        Route::resource('footer-section', 'FooterSectionController');
        Route::resource('student-zone', 'StudentZoneController');
        Route::resource('club', 'ClubController')->only(['index', 'edit', 'update']);

        // Content Section Routes (Generic for all section headings) - Protected by auth middleware
        Route::get('content-section/{key}', 'ContentSectionController@manage')->name('content-section.manage');
        Route::post('content-section/{key}', 'ContentSectionController@store')->name('content-section.store');
        Route::delete('content-section/{key}', 'ContentSectionController@destroy')->name('content-section.destroy');

        // Footer Links Routes
        Route::post('footer-section/{section}/link', 'FooterSectionController@storeLink')->name('footer-section.storeLink');
        Route::patch('footer-link/{link}', 'FooterSectionController@updateLink')->name('footer-section.updateLink');
        Route::delete('footer-link/{link}', 'FooterSectionController@destroyLink')->name('footer-section.destroyLink');
    });
});
// Fees Routes
// Route::get('fees', 'FeesController@index')->name('student.fees.index');
// Route::get('fees/pay/{id}', 'FeesController@pay')->name('student.fees.pay');
