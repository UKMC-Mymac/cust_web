...
@section('title', $page->display_text ?? $page->title)
@section('content')
...
{{-- HERO --}}
{{-- <div class="page-hero">
    <div class="page-hero-cut">
        <svg viewBox="0 0 1440 48" preserveAspectRatio="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M0,48 L1440,0 L1440,48 Z" fill="#f2f6fa"/>
        </svg>
    </div>
    <div class="container page-hero-inner">
        <div class="page-kicker">Page</div>
...
<section class="page-shell">
    <div class="container">
        <div class="page-content-wrap">
            @if(($page->layout_mode ?? 'editor') === 'builder' && !empty($builderSections))
                @php
                ...
                @if($pageNavItems->isNotEmpty())
                    <div class="page-card-with-sidebar {{ $pageNavPosition === 'left' ? 'nav-left' : 'nav-right' }}">
                        @if($pageNavPosition === 'left')
                                ...
                                <div class="page-builder-nav-wrap">
                                    <div class="page-builder-menu-grid">
                                        @foreach($pageNavItems as $navItem)
                                            ...
                                            @endphp
                                            @if($linkedPage || !empty($navItem['title']))
                                                @if($linkedPage)
                                                    <a class="page-builder-menu-card @if(!$navItem['active']) is-inactive @endif @if($isActivePage) active @endif" href="{{ $navHref }}" @if($isActivePage) aria-current="page" @endif>
                                                        <span class="title">{{ $navItem['title'] ?: ($linkedPage->display_text ?? $linkedPage->title) }}</span>
                                                    </a>
                                                @else
                        ...
                        <div class="page-card"> 
                            ...
                            <div class="page-card-body">
                                @include('web.page-builder', ['builderSections' => $builderSections, 'pagesById' => $pagesById, 'renderSectionNavInside' => false])
                            </div>
                        </div>

                        @if($pageNavPosition === 'right')
                            <aside class="page-builder-nav-column page-builder-page-sidebar">
                                <div class="page-builder-nav-wrap">
                                    <div class="page-builder-menu-grid">
                                        @foreach($pageNavItems as $navItem)
                ...
                @else
                    <div class="page-card">
                        <div class="page-card-bar"></div>
                        <div class="page-card-body">
                            @include('web.page-builder', ['builderSections' => $builderSections, 'pagesById' => $pagesById])
                        </div>
                    </div>
                @endif
            @else
