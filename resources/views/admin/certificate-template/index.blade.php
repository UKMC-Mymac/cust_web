...
@section('page_css')
    ...
    <link rel="stylesheet" type="text/css" href="{{ asset('css/prints/certificate.css') }}" media="screen">
@endsection
                            ...
                            <table id="basic-table" class="display table nowrap table-striped table-hover" style="width:100%">
                                <thead>
                                    <tr>
                                        ...
                                        <th>{{ __('field_logo_left') }}</th>
                                        <th>{{ __('field_border_background') }}</th>
                                ...
                                <tbody>
                                  @foreach( $rows as $key => $row )
                                    <tr>
                                        ...
                                        <td>
                                            @if(isset($row->background) && is_file('uploads/'.$path.'/'.$row->background))
                                            <img src="{{ asset('uploads/'.$path.'/'.$row->background) }}" class="img-fluid" style="max-width: 80px; max-height: 80px;">
                                            @endif
                                        </td>
                                        <td>
                                            @if( $row->status == 1 )
