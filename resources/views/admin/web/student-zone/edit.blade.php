            ...
            <div class="col-md-6 mx-auto">
                <div class="card">
                    <div class="card-header bg-primary text-white">
                        <h5 class="mb-0">
                            <i class="fas fa-edit"></i> Edit Zone: {{ $row->title }}
                        </h5>
                    </div>
                    ...
                    <form method="POST" action="{{ route($route.'.update', $row->id) }}" enctype="multipart/form-data">
                        ...
                        <div class="card-block">
                            ...
                            <div class="form-group mb-3">
                                ...
                                <input type="text" class="form-control @error('title') is-invalid @enderror" id="title" name="title" value="{{ old('title', $row->title) }}" required>
                                @error('title')
                            ...
                            <div class="form-group mb-3">
                                ...
                                @if($row->icon_url)
                                    <div class="mb-2">
                                        ...
                                        <img src="{{ asset('uploads/student-zone/' . $row->icon_url) }}" alt="icon" style="max-width: 60px; max-height: 60px;">
                                    </div>
                                @endif
                                <input type="file" class="form-control @error('icon_url') is-invalid @enderror" id="icon_url" name="icon_url" accept="image/*">
                                <small class="text-muted">Upload a PNG or JPG image to change. Leave blank to keep current icon</small>
                                @error('icon_url')
                                    <span class="invalid-feedback d-block" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
                            ...
                            <div class="form-group mb-3">
                                ...
                                <input type="number" class="form-control @error('order') is-invalid @enderror" id="order" name="order" value="{{ old('order', $row->order) }}">
