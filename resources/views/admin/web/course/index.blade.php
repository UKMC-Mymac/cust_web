            ...
            <div class="col-md-12">
                <div class="card border-info">
                    ...
                    <div class="card-block">
                        @php
                            $CourseSection = \App\Models\Web\ContentSection::query()
                                                                           ->where('key', 'course')
                                                                           ->first();
                        @endphp
                        <div class="row">
                            <div class="col-md-8">
                                <div class="mb-2">
                                    ...
                                    <span class="text-muted">{{ $CourseSection->subtitle ?? 'Not set' }}</span>
                                </div>
                                <div class="mb-2">
                                    <strong>Title:</strong> 
                                    <span class="text-muted">{{ $CourseSection->title ?? 'Not set' }}</span>
                                </div>
                                <div>
                                    <strong>Description:</strong> 
                                    <span class="text-muted">{!! str_limit($CourseSection->description ?? 'Not set', 100) !!}</span>
                                </div>
                            </div>
                            <div class="col-md-4 text-end">
                                @if($CourseSection && $CourseSection->status == 1)
                                ...
                                @endif
                                <a href="{{ route('admin.content-section.manage', 'course') }}" class="btn btn-info">
                                ...
                                <tbody>
                                  @foreach( $rows as $key => $row )
                                    <tr>
                                        ...
                                        <td>{{ $row->department->title ?? 'N/A' }}</td>
                                        <td>
