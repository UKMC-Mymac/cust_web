                        ...
                        <div class="row">
                            <div class="col-md-8">
                                <div class="mb-2">
                                    ...
                                    <span class="text-muted">{{ $news_eventSection->subtitle ?? 'Not set' }}</span>
                                </div>
                                <div class="mb-2">
                                    <strong>Title:</strong> 
                                    <span class="text-muted">{{ $news_eventSection->title ?? 'Not set' }}</span>
                                </div>
                                <div>
                                    <strong>Description:</strong> 
                                    <span class="text-muted">{!! str_limit($news_eventSection->description ?? 'Not set', 100) !!}</span>
                                </div>
                            </div>
                            <div class="col-md-4 text-end">
                                @if($news_eventSection && $news_eventSection->status == 1)
                                ...
                                @endif
                                <a href="{{ route('admin.content-section.manage', 'news-event') }}" class="btn btn-info">
                    ...
                    <div class="card-block">
                        <form class="needs-validation" novalidate method="get" action="{{ route($route.'.index') }}">
                            <div class="row gx-2">
                                ...
                                <div class="form-group col-md-3">
                                    <button type="submit" class="btn btn-info btn-filter" style="margin-top: 25px;"><i class="fas fa-search"></i> {{ __('btn_search') }}</button>
                                </div>
                            </div>
                        </form>
                    </div>
                                ...
                                <tbody>
                                  @foreach( $rows as $key => $row )
                                    <tr>
                                        ...
                                        <td>
                                            @if( !empty($row->pinned) )
                                            <span class="badge badge-pill badge-warning text-dark"><i class="fas fa-thumbtack"></i> {{ $row->pinned }}</span>
                                            @else
                                            -
                                            @endif
                                        </td>
