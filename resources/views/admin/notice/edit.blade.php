                    ...
                    <div class="card-block">
                      <div class="row">
                        ...
                        <div class="form-group col-md-4">
                            <label for="category">{{ __('field_category') }} <span>* ({{ __('select_multiple') }})</span></label>
                            <select class="form-control select2" name="categories[]" id="category" multiple required>
                                @foreach( $categories as $category )
                                <option value="{{ $category->id }}" @if(in_array($category->id, $row->categories->pluck('id')->toArray())) selected @endif>{{ $category->title }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group col-md-4">
                            ...
                            <input type="text" class="form-control autonumber" name="notice_no" id="notice_no" value="{{ $row->notice_no }}" required data-v-max="999999999" data-v-min="0">
